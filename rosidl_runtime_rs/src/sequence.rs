...
///
/// The layout of a concrete `Sequence<T>` is the same as the corresponding
/// `Sequence` struct generated by `rosidl_generator_c`. For instance,
/// `rosidl_runtime_rs::Sequence<rosidl_runtime_rs::String>` is the same
/// as `std_msgs__msg__String__Sequence`. See the [`Message`](crate::Message)
/// trait for background information on this topic.
///
///
/// # Example
///
...
/// A bounded sequence.
///
/// The layout of a concrete `BoundedSequence<T>` is the same as the
/// corresponding `Sequence` struct generated by `rosidl_generator_c`. For
/// instance, `rosidl_runtime_rs::BoundedSequence<rosidl_runtime_rs::String>`
/// is the same as `std_msgs__msg__String__Sequence`, which also represents both
/// bounded sequences.  See the [`Message`](crate::Message) trait for background
...
/// A by-value iterator created by [`Sequence::into_iter()`] and
/// [`BoundedSequence::into_iter()`].
...
// SAFETY: A sequence is a simple data structure, and therefore not
// thread-specific.
...
impl<T: SequenceAlloc> Iterator for SequenceIterator<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let elem = unsafe {
            ...
            let elem = ptr.read();
            // Need to make sure that dropping the sequence later will not fini() the
            // elements
...
/// Bounded sequences are created by additionally specifying the maximum
/// capacity (the `N` type parameter) in the beginning, followed by a `#`.
///
/// # Example
/// ```
