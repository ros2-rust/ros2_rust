...
///
/// > The identifier uniquely identifies the publisher for the local context,
/// > but
/// it will not necessarily be the same identifier given in other contexts or
/// processes for the same publisher.
/// Therefore the identifier will uniquely identify the publisher within your
/// application but may disagree about the identifier for that publisher when
/// compared to another application.
/// Even with this limitation, when combined with the publisher sequence number
/// it can uniquely identify a message within your local context.
/// Publisher GIDs generated by the RMW implementation could collide at some
/// point, in which case it is not possible to distinguish which publisher sent
/// the message. The details of how GIDs are generated are RMW implementation
/// dependent.
///
/// > It is possible the the RMW implementation needs to reuse a publisher GID,
/// due to running out of unique identifiers or some other constraint, in which
/// case the RMW implementation may document what happens in that case, but that
/// behavior is not defined here.
/// However, this should be avoided, if at all possible, by the RMW
/// implementation, and should be unlikely to happen in practice.
...
pub struct PublisherGid {
    ...
    ///
    /// It is not converted to a [`CString`][1], since most people who request a
    /// `MessageInfo` do not need it.
    ///
    /// [1]: std::ffi::CString
    pub implementation_identifier: *const std::os::raw::c_char,
}

// SAFETY: The implementation identifier doesn't care about which thread it's
// read from.
...
pub struct MessageInfo {
    ...
    ///
    /// The `rmw` layer does not specify the exact point at which the RMW
    /// implementation must take the timestamp, but it should be taken
    /// consistently at the same point in the process of publishing a
    /// message.
    ...
    /// The `rmw` layer does not specify the exact point at which the RMW
    /// implementation must take the timestamp, but it should be taken
    /// consistently at the same point in the process of receiving a
    /// message.
    pub received_timestamp: Option<SystemTime>,
    /// Sequence number of the received message set by the publisher.
    ///
    /// This sequence number is set by the publisher and therefore uniquely
    /// identifies a message when combined with the publisher GID.
    /// For long running applications, the sequence number might wrap around at
    /// some point.
    ///
    /// If the RMW implementation doesn't support sequence numbers, its value
    /// will be [`u64::MAX`].
    ///
    /// Requirements:
    ///
    /// If `psn1` and `psn2` are the publication sequence numbers received
    /// together with two messages, where `psn1` was obtained before `psn2`
    /// and both sequence numbers are from the same publisher (i.e. also
    /// same publisher gid), then:
    ///
    /// - `psn2 > psn1` (except in the case of a wrap around)
    /// - `psn2 - psn1 - 1` is the number of messages the publisher sent in the
    ///   middle of both received messages. Those might have already been taken
    ///   by other messages that were received in between or lost. `psn2 - psn1
    ///   - 1 = 0` if and only if the messages were sent by the publisher
    ///   consecutively.
    ...
    /// Requirements:
    ///
    /// If `rsn1` and `rsn2` are the reception sequence numbers received
    /// together with two messages, where `rsn1` was obtained before `rsn2`,
    /// then:
    ///
    /// - `rsn2 > rsn1` (except in the case of a wrap around)
    /// - `rsn2 = rsn1 + 1` if and only if both messages were received
    ///   consecutively.
