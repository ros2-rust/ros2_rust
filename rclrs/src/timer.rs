use crate::{
    ...
    ToLogParams, ToResult, ENTITY_LIFECYCLE_MUTEX, Node, RclPrimitive, RclPrimitiveKind,
    RclPrimitiveHandle, Worker, WorkerCommands, WorkScope, Waitable, WaitableLifecycle,
};
...
impl<Scope: WorkScope> TimerState<Scope> {
    ...
    /// * [`Self::set_oneshot`]
    /// * [`Self::set_inert`].
    pub fn set_callback(&self, callback: AnyTimerCallback<Scope>) -> Option<AnyTimerCallback<Scope>> {
...
impl TimerState<Node> {
    ...
    /// * [`Self::set_repeating`]
    /// * [`Self::set_inert`]
    pub fn set_oneshot<Args>(&self, f: impl IntoNodeTimerOneshotCallback<Args>) -> Option<AnyTimerCallback<Node>> {
...
mod tests {
    ...
    fn test_execute_when_is_not_ready() {
        ...
        unsafe { executable.execute(&mut ()).unwrap(); }
        assert!(!executed.load(Ordering::Acquire));
    }
