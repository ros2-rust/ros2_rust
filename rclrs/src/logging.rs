...
mod tests {
    ...
    fn test_rosout_publishing_default() -> Result<(), RclrsError> {
        ...
        let inner_count = Arc::clone(&received_count);
        ...
        let _subscription = node.create_subscription::<Log, _>(
            ...
            move |msg: Log| {
                ...
                if msg.name.to_string().contains("rosout_default_node") {
                    ...
                    inner_count.fetch_add(1, Ordering::Release);
                }
            },
        )?;

        // Log a dummy message first to ensure rosout publisher is created (lazy init on non-Humble)
        // and give time for publisher-subscriber discovery
        log!(node.info(), "Initializing rosout");
        std::thread::sleep(Duration::from_millis(200));

        // Debug: Check topic state after logging
        let topics = node.get_topic_names_and_types()?;
        println!(
            "DEBUG: Topics after logging: {:?}",
            topics.keys().collect::<Vec<_>>()
        );
        println!(
            "DEBUG: /rosout publishers: {:?}",
            node.count_publishers("/rosout")
        );
        println!(
            "DEBUG: /rosout subscriptions: {:?}",
            node.count_subscriptions("/rosout")
        );
        ...
        // Spin until all 5 messages are received (1 init + 4 test messages, with timeout)
        let start_time = std::time::Instant::now();
        while received_count.load(Ordering::Acquire) < 5 {
            let _ = executor.spin(SpinOptions::spin_once().timeout(Duration::from_millis(100)));
